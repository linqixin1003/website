...
def add_hero_image_to_file(file_path, image_path):
    """为HTML文件添加头图"""
    try:
        ...
        # 查找header标签并添加头图
        header_pattern = r'(<header[^>]*>)\s*(<h1[^>]*>.*?</h1>)'
        
        def replace_header(match):
            header_open = match.group(1)
            h1_tag = match.group(2)
            
            # 提取h1标签中的文本作为alt属性
            h1_text_match = re.search(r'<h1[^>]*>(.*?)</h1>', h1_tag)
            alt_text = h1_text_match.group(1) if h1_text_match else "Rock Image"
            
            hero_image = f'''        <div class="hero-image">
            <img src="../../images/rock/{image_path}" alt="{alt_text}" style="width: 100%; height: 200px; object-fit: cover; border-radius: 8px;">
        </div>'''
        ...
        if new_content != content:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"✅ 已为 {file_path} 添加头图")
            return True
        else:
            print(f"⚠️  {file_path} 未找到匹配的header标签")
            return False
...
def main():
    ...
    print(f"开始为 {total_count} 个岩石文章添加头图...")
    
    for file_path, image_path in image_mappings.items():
        if os.path.exists(file_path):
            if add_hero_image_to_file(file_path, image_path):
                success_count += 1
        else:
