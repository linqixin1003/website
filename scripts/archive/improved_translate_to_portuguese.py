...
def translate_text_improved(text):
    ...
    for english in sorted_keys:
        ...
        if re.search(pattern, text, re.IGNORECASE):
            text = re.sub(pattern, translation_map[english], text, flags=re.IGNORECASE)
    
    return text

def copy_and_translate_file_improved(src_file, dest_file):
    """改进的文件翻译函数"""
    ...
    try:
        ...
        def translate_title(match):
            ...
            translated = translate_text_improved(title_content)
                ...
                if '<span' in text_content or '<a' in text_content or '<strong' in text_content or '<em' in text_content:
                    ...
                    for part in parts:
                        if part.startswith('<'):
                            ...
                            translated_parts.append(translate_text_improved(part))
                    translated_text = ''.join(translated_parts)
                else:
                    translated_text = translate_text_improved(text_content)
        ...
        def translate_comment(match):
            ...
            translated = translate_text_improved(comment_text)
            return f'<!-- {translated} -->'
...
def main():
    ...
    for html_file in html_files:
        ...
        copy_and_translate_file_improved(html_file, dest_file)
    
    print(f"\n🎉 重新翻译完成！共处理了 {len(html_files)} 个文件")
    print(f"英文文件已重新翻译成葡萄牙语并保存到 pt/ 目录")
